...
export const roadmap: Roadmap = {
  tasks: [
    ...
    },
    {
      ...
      notes: [
        ...
        'Outside-disc-face to outside-disc-face: 1.415 m → axle flange-to-flange ~1.397–1.401 m (lock exact rotor + caliper-bracket stack before axle order)',
        'Wheel offset: +38 mm (matches purchased wheels)',
        'Current housing measured ~60.5″ flange-to-flange (~1537 mm) — needs ~136–140 mm narrowing',
        'Spline: 31-spline locked; bolt pattern 4×100; custom axles + custom flanges',
        'U-joint: 1310 vs 1330 — defer to driveshaft shop',
        'Moser contacted to spec axles against the locked track + current housing',
      ],
    },
    {
      id: 't-scan-rear',
    ...
    },
    {
      ...
      epic: 'research',
      status: 'complete',
      summary: 'Initial LCA/UCA inner pivots, BJ centers, rack mounts, and hub face dimensions pulled from forum-published Miata reference drawings and photo-CAD. Superseded by the donor subframe scan (t-scan-donor-miata) after the forum CAD produced an 11.5° caster — UCA pickups and LCA inner-pivot width were wrong in the forum data. LCA arm geometry (fa span, BJ offset, z rise) was correct and is retained.',
      notes: [
        'Source: aggregated online references for NA Miata front geometry',
        'Outputs: pivot coords, BJ centers, rack mounts, hub face (initial pass)',
        'Superseded by donor scan: UCA pickups + LCA width corrected; LCA arm geometry retained',
      ],
    },
    {
      id: 't-scan-donor-miata',
      title: '3D Scan: Donor Miata Front Subframe',
      epic: 'research',
      status: 'complete',
      summary: 'Einstar 2 scan of the donor front subframe in the shop. The forum-published pickup-point CAD we had been driving SA V2 on yielded an absurd 11.5° static caster — clearly wrong, triggered the scan as a sanity check. Scan invalidated the forum CAD: UCA inner-pivot positions were off, and the LCA inner-pivot width (lca_from_cl) was wrong. LCA arm geometry (fa span, BJ offset) is unchanged. Downstream: SUSP constants in plot_geometry_reference.py, narrowing-analysis, and geometry-design-reference all need to be updated with the scanned numbers.',
      notes: [
        'Trigger: forum-CAD pickup points produced 11.5° static caster — implausible for a street-car Miata (~5° expected)',
        'Tool: Einstar 2',
        'Findings: UCA pickups wrong, LCA inner-pivot width wrong',
        'Unchanged: LCA arm geometry (lca_fa_span, lca_bj_fa, lca_z_rise)',
        'Follow-up: update SUSP constants and re-generate pickup/RC/caster plots',
      ],
    },
    ...
    },
    {
      ...
      notes: [
        '1230 mm front track locked for downstream design',
        'Validated against scanned fender geometry (t-scan-front)',
        'Caveat: minor interference at full lock + full compression — to be confirmed at mock-up and likely absorbed by a steering-lock stop rather than re-narrowing the track',
      ],
    },
    {
    ...
    },
    {
      ...
      notes: [
        ...
        'Main blocker: time',
        'Inputs: scanned donor pickup points from t-scan-donor-miata (forum-CAD numbers superseded — UCA pickups + LCA width were wrong)',
        ...
        'Action item: update SUSP constants in scripts/plot_geometry_reference.py with scanned UCA pickups + corrected lca_from_cl, then regenerate pickup/RC/caster plots',
      ],
    },
    {
    ...
    },
    {
      ...
      epic: 'fabrication',
      status: 'in-progress',
      summary: 'Phase 1 bench stack (Pi 5 + Freenove NVMe + Waveshare DSI on the 27W USB-C PSU) verified, then Phase 2 swap to the PiCAN FD Duo Isolated HAT + Mini-Box DCDC-USB for the automotive 12V / ignition-shutdown chain. The FD Duo is non-SMPS — the Pi is powered directly from the Mini-Box DCDC-USB, not through the HAT.',
      notes: [
        'Phase 1: Pi 5 / NVMe / DSI screen running off bench PSU — proves OS + screen + boot',
        'Phase 2: drop USB-C, add PiCAN FD Duo Isolated HAT; Pi 5 V comes directly from the Mini-Box DCDC-USB; clean 12 V from the DCDC-USB powers the screen and biases the HAT-side; DCDC-USB also handles ~45 s graceful shutdown on ign-off',
        'Dual CAN bringup (can0 Haltech @ 1 Mbps, can1 Toyota EPAS @ 500 kbps) tracked separately as t-dash-dual-can-bringup',
    ...
    },
    {
      ...
      status: 'planning',
      kind: 'purchase',
      summary: 'Toyota Prius Gen 3 (2010–2015) column-mount EPAS assembly — column + integrated controller. Any year in that range works; they share the same powertrain CAN architecture, so the gateway is donor-year-agnostic across Gen 3.',
    },
    {
    ...
    /* ─────────── EPAS CAN gateway (dual-bus, Pi-driven) ─────────── */
    {
      id: 't-buy-can-hat-duo',
      title: 'Buy: PiCAN FD Duo Isolated HAT',
      epic: 'fabrication',
      status: 'complete',
      kind: 'purchase',
      summary: 'Purchased: PiCAN FD Duo Isolated (non-SMPS) from Copperhill. Two galvanically-isolated CAN channels: can0 (Haltech, 1 Mbps) + can1 (Toyota Prius column, 500 kbps). Pi gets 5 V directly from the Mini-Box DCDC-USB.',
      posts: ['epas-can-gateway'],
    },
    {
      id: 't-dash-fdduo-swap',
      title: 'Dash Hardware: Dual CAN Bringup',
      epic: 'fabrication',
      status: 'planning',
      summary: 'Bring up both CAN channels on the PiCAN FD Duo Isolated: device-tree overlays for MCP251xFD on SPI0 + SPI1, can0 @ 1 Mbps for the Haltech side, can1 @ 500 kbps for the Toyota EPAS side. Validate with loopback + a recorded Haltech log.',
      posts: ['epas-can-gateway'],
      notes: [
        'Overlays: mcp251xfd on SPI0 (can0) and SPI1 (can1) with correct oscillator + interrupt pins',
        'Validate: `ip -details link show can0 / can1` reports correct bitrates and ERROR-ACTIVE',
        'Pi power topology: HAT does NOT feed Pi over GPIO — 5 V comes directly from Mini-Box DCDC-USB',
      ],
    },
    {
      id: 't-epas-gateway-software',
      title: 'EPAS CAN Gateway Service',
      epic: 'design',
      status: 'planning',
      posts: ['epas-can-gateway'],
      summary: 'Lightweight C/Python systemd service translating Haltech → Toyota frames. Reads RPM (Haltech 0x360, bytes 0-1) and speed (Haltech 0x370, bytes 0-1) on can0; writes Toyota 0x1C4 Powertrain (50 Hz) and 0xB4 Speed (20 Hz) on can1 with the 8-bit rolling checksum on byte 7. Runs outside the Godot dash app so a UI crash never costs power steering.',
      notes: [
        'Separate systemd unit, Restart=always, WatchdogSec set',
        'Speed conversion: Haltech km/h × 10 → mph → Toyota DBC scale 0.0062',
        'Checksum: ((id>>8) + (id&0xFF) + DLC + bytes 0..6) & 0xFF',
        'No UI dependency — logs to journald only',
        'Bench-developable against recorded Haltech logs replayed on vcan0',
      ],
    },
    {
      id: 't-epas-gateway-bench',
      title: 'EPAS Gateway Bench Validation',
      epic: 'fabrication',
      status: 'planning',
      posts: ['epas-can-gateway'],
      summary: 'Bench rig: FD Duo on Pi, recorded Haltech log replayed on can0, Prius column wired to can1 with 12 V supply. Confirm column wakes up (RPM > 0), confirm assist scales with spoofed speed by hand-loading the torque sensor. Pass criterion: assist character matches what the column does in a real Prius across the speed range.',
      notes: [
        'Requires t-buy-epas (column on hand), t-buy-can-hat-duo, t-dash-fdduo-swap, t-epas-gateway-software',
        'Catch failsafe-fallback behaviour: kill the gateway service and confirm column falls back cleanly to fixed assist',
      ],
    },
    ...
    },
    {
      ...
      kind: 'decision',
      status: 'decided',
      decision: 'Dual-bus CAN gateway on the dash Pi: rebroadcast Haltech RPM + speed as Toyota 0x1C4 + 0xB4 frames so the Prius column delivers speed-sensitive assist. Failsafe-mode wiring (12 V + ignition only) is the fallback if the gateway service is down.',
      posts: ['epas-can-gateway'],
      summary: 'Failsafe-mode column EPAS (default, simple) vs ECU-controlled assist over CAN. Chosen: gateway. Implementation lives outside the Godot dash app as a separate systemd service for safety isolation.',
      notes: [
        'Gateway runs as its own systemd service — NOT inside the Godot dash app (UI crash != lose power steering)',
        'Hardware: PiCAN FD Duo Isolated (non-SMPS) HAT replacing the single-channel PiCAN3',
        'Toyota frames: 0x1C4 Powertrain (RPM, 50 Hz) + 0xB4 Speed (20 Hz), with 8-bit rolling checksum on byte 7',
        'Failure mode: column falls back to internal failsafe (fixed assist) — same as no-gateway baseline',
      ],
    },
    {
  ...
  edges: [
    ...
    { from: 't-scan-donor-miata',   to: 't-sa-v2',                kind: 'hard', label: 'Scanned pickup coords supersede forum CAD.' },
    ...
    /* EPAS CAN gateway chain (dual-bus Pi). */
    { from: 't-decide-epas-control', to: 't-buy-can-hat-duo',      kind: 'hard' },
    { from: 't-decide-epas-control', to: 't-epas-gateway-software', kind: 'hard' },
    { from: 't-buy-can-hat-duo',    to: 't-dash-fdduo-swap',       kind: 'hard' },
    { from: 't-dash-hardware',      to: 't-dash-fdduo-swap',       kind: 'hard', label: 'FD Duo swap is Phase 3 of dash hardware.' },
    { from: 't-dash-fdduo-swap',    to: 't-epas-gateway-bench',    kind: 'hard' },
    { from: 't-epas-gateway-software', to: 't-epas-gateway-bench', kind: 'hard' },
    { from: 't-buy-epas',           to: 't-epas-gateway-bench',    kind: 'hard', label: 'Column needs to be on the bench for assist validation.' },
    { from: 't-decide-ecu',         to: 't-epas-gateway-software', kind: 'hard', label: 'Haltech CAN broadcast spec defines the input frames.' },
    { from: 't-epas-gateway-bench', to: 't-epas-install',          kind: 'hard', label: 'Gateway must be bench-validated before in-car install.' },
    { from: 't-dash-fdduo-swap',    to: 't-dash-install',          kind: 'soft', label: 'FD Duo swap can land before or after dash physical install.' },
