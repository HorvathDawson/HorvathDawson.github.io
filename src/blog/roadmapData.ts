...
export const roadmap: Roadmap = {
  tasks: [
    ...
    },
    {
      ...
      kind: 'decision',
      status: 'decided',
      decision: 'Miata NA rack (depowered, 175 mm narrowed) + 2010 Toyota Prius column-mount EPAS, driven by the dash Pi acting as a dual-bus CAN gateway (Haltech → Toyota frames) for speed-sensitive assist. Failsafe-mode wiring is the fallback.',
      ...
      notes: [
        ...
        'Column: 2010 Toyota Prius (Gen 3 brushless) — chosen for its CAN-controlled speed-sensitive assist',
    ...
    },
    {
      ...
      epic: 'fabrication',
      status: 'in-progress',
      summary: 'Phase 1 bench stack (Pi 5 + Freenove NVMe + Waveshare DSI on the 27W USB-C PSU) verified, then Phase 2 swap to PiCAN3 HAT + Mini-Box DCDC-USB for the automotive 12V / ignition-shutdown chain. Phase 3 (gateway-ready) swaps the PiCAN3 for a PiCAN FD Duo Isolated when the EPAS gateway lands.',
      notes: [
        ...
        'Phase 3: swap PiCAN3 → PiCAN FD Duo Isolated (non-SMPS); Pi now drinks 5 V directly from Mini-Box. Tracked separately as t-buy-can-hat-duo + t-dash-fdduo-swap',
    ...
    },
    {
      ...
      status: 'planning',
      kind: 'purchase',
      summary: '2010 Toyota Prius (Gen 3) column-mount EPAS assembly — column + integrated controller. Chosen for the CAN-controlled speed-sensitive assist driven by the dash Pi gateway.',
    },
    {
    ...
    /* ─────────── EPAS CAN gateway (dual-bus, Pi-driven) ─────────── */
    {
      ...
      status: 'planning',
      kind: 'purchase',
      summary: 'PiCAN FD Duo Isolated (non-SMPS) from Copperhill — replaces the single-channel PiCAN3 in the dash stack. Provides two galvanically-isolated CAN channels: can0 (Haltech, 1 Mbps) + can1 (Toyota Prius column, 500 kbps).',
      posts: ['epas-can-gateway'],
    },
    {
      id: 't-dash-fdduo-swap',
      title: 'Dash Hardware: Swap PiCAN3 → FD Duo',
      epic: 'fabrication',
      status: 'planning',
      summary: 'Phase 3 hardware swap: remove PiCAN3, install PiCAN FD Duo Isolated, re-route Pi 5 V to be fed directly from the Mini-Box DCDC-USB (FD Duo is non-SMPS). Update device-tree overlays for MCP251xFD on both channels and bring up can0 + can1.',
      posts: ['epas-can-gateway'],
      notes: [
        'Pi power topology changes: HAT no longer feeds Pi via GPIO — direct from DCDC-USB',
        'New overlays: mcp251xfd on SPI0 (can0) and SPI1 (can1), correct oscillator + interrupt pins',
